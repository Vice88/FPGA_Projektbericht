...
\begin{itemize}
	...
	\begin{itemize}
		\item change the HDD constant in FPGACommunication/PcFPGACommunication.cpp and FaultConfiguration/IRA.cpp to the drive where your quartus is installed	
	\end{itemize}
	\item "Inconsistent set or reset compile started variable", cf. [\textbf{figureXYZ}]
	\begin{itemize}
...
\begin{enumerate}
	...
	\item driver installation of FTDI Chip for USB-Serial converter
	\item error in the design flow
	\item errors while execution
	\begin{itemize}
		\item "inconsistant set or reset of compile started variable" [ - troubleshooting takes a long time. This is not reported in altera forum nor the handbook. ]
		\item "quartus\textunderscore pgm could not be found" - [insert SCREENSHOT]
	\end{itemize}
\end{enumerate}
...
\end{figure}
 The solution to this problem was found on: \textbf{\textit{http://altera-guide.blogspot.de/2012/11/many-collage-students-find-ourselves.htmlcomment-form}}
\begin{figure}[ht]
	\centering
  \includegraphics[width=1\textwidth, angle=0]{installationGuide.png}
	...
	\label{installguide}
\end{figure}
After installing the driver, another problem was detected while starting the driver:
\begin{figure}[ht]
	\centering
  \includegraphics[width=1\textwidth, angle=0]{USB_blaster_problem.png}
	...
	\label{usbblaster1}	
\end{figure}
The attempt to install windows standard driver results in the following error:
\begin{figure}[ht]
	\centering
  \includegraphics[width=1\textwidth, angle=0]{USB_blaster_problem2.png}
	...
	\label{usbblaster2}	
\end{figure}
Also there were this error:
\begin{enumerate}
	\item driver installation of FTDI chip
\end{enumerate}
...
\begin{enumerate}
	\item error while execution - inconsistent set...
\end{enumerate}
...
\end{itemize}
Further improvements for future work is to setup the program on a virtual machine to become independent of operating system and thus driver installation. Also a version control for this program would be great for future works and a nice structure of the project. In this project is no structure and it makes it very difficult to understand whats going on.\\This leads to the idea of redoing the whole project and setting it up to an independent machine like a virtual machine with a version control of the project.
