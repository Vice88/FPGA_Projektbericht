...
The software used in both projects were Quartus II Version 14.1/15.0 and QSYS on Windows 7/8.1.\\Quartus II is needed to generate VHDL- or Verilog programs which can be used by a fpga. The software can be configured to reach different design goals, e.g. time complexity, speed, optimization of compilation time or energy consumption.\\ QSYS is described as a tool for system integration to safe time by generating automatic connection-logic of different components. The developed VHDL code from Quartus II software can be imported to QSYS to connect different components, e.g. on the DE-SoC1. For an example QSYS has to initiate, configure and connect the HPS to the FPGA if they need to communicate.\\ USB driver for USB Blaster is needed to program the fpga with the computer.

\section{Partial reconfiguration}
\subsection{Prerequisites for the project of partial reconfiguration}
...
\subsection{Problems during the first project of partial reconfiguration}
\textbf{Partial reconfiguration is not supported by the Cyclone V fpga}. Following problems occurred:
\begin{itemize}
...
\textbf{Hardware} used in this project was a former dev kit from altera with a fpga Cyclone IV.\\
In addition to the former mentioned software there is some \textbf{special Software} required for this project:
...
\begin{itemize}
	...
	\item USB-to-Serial driver $\rightarrow$ get a connection from dev kit to pc via RS232
\end{itemize}
...
\begin{figure}[ht]
  ...
  \includegraphics[width=1\textwidth, angle=0]{installationGuide.png}
	...
	\label{fig1}
\end{figure}
Although it is an official driver from altera i was struggling with this problem a few days to find out, that windows 8.1 does not support this official driver. The solution is found on: \textbf{\textit{http://altera-guide.blogspot.de/2012/11/many-collage-students-find-ourselves.htmlcomment-form}} \vspace{0.5cm}
...
A lot of problems occurred during the work on the project. These problems prevented that vhdl could be learned, which was the intend to do a project at the chair of computer architecture. The second project was nearly only a project in the programming language of C++ and no more hardware programming.
...
\section{Appendix}
\subsection{ A - Problems and Errors}
\begin{figure}[ht]
	\centering
