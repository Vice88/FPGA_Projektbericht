...
\subsection{Problems}
Here are some problems which occur during seting-up or execution of the project.
\begin{enumerate}
	\item driver installation of USB blaster II and FTDI Chip for USB-Serial converter
	\item error in design flow
	\item common error while execution: inconsistent set and quartus could not be fond
\end{enumerate}
First of all i encountered the problem of driver installation. Although it seems to be trivial, if you encounter this problem: 
\begin{figure}[ht]
	\centering
...
The solution for the inconsistent set would be to clear out 'db' and 'incremental db' in 'FaultInjectiontester restored' folder and recompile the quartus project. If a 'quartus' execution file could not be found, you have to check the paths entered in FPGACommunication.cpp and IRA.cpp and change them.
...
There are a lot of problems which occured during the whole project, which make it impossible to do great in programing. The workstation for this project was my desktop pc and not my laptop. So this make it difficult for solution finding. I come to a point where i decided to redo the whole project, but this would expand the project time to an inappropriate scale. In the workload of 180h for 6 ECTS, a few are gone to the wasted time on partial reconfiguration, like literature survey, tutorial on Quartus II and QSYS software. Do the design flow, understand everything took over eight weeks and a workload of app. 80h. After changing to the project of fault simulation, the most of the time was to understand the former work of this project. This took a very long time, because it is so complicated and not documented and not trivial. Also there was a huge error in the design flow, which took very long to detect. Than there was the problems discussed earlier. Debugging and troubleshooting sadly become the main part of the project. 
